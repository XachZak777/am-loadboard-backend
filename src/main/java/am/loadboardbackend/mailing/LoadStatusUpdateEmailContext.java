...
public class LoadStatusUpdateEmailContext extends AbstractEmailContext {
    ...
    public void init(String brokerEmail, String brokerName, LoadPosting load,
                     Carrier carrier, String newStatus, String fromAddress, String appBaseUrl) {
        setTo(brokerEmail);
        setFrom(fromAddress);
        ...
        setSubject("Load Update: " + statusLabel + " — " + vehicleSummary);
    ...
    private String descriptionFor(String status, String carrierName, String vehicle) {
        return switch (status) {
            case "PICKED_UP" -> carrierName + " has confirmed the " + vehicle + " has been picked up and is in transit.";
            case "DELIVERED" -> carrierName + " has confirmed the " + vehicle + " has been successfully delivered.";
